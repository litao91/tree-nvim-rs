...
local is_macos = not is_windows and fn.has('win32unix') == 0 and
                     fn.has('macunix') == 1
local is_linux = fn.has('unix') == 1 and fn.has('macunix') == 0 and
                     fn.has('win32unix') == 0
...
function M.resume(bufnrs)
    if bufnrs == nil then return end
    if type(bufnrs) == 'number' then bufnrs = {bufnrs} end
    ...
    if etc.split == 'tab' then cmd 'tabnew' end
    ...
    if etc.split ~= 'floating' then
        ...
        if etc.direction == 'botright' then direction = 'botright' end
        str = string.format("silent keepalt %s %s %s %d", direction, vertical,
                            command, bufnr)
...
function M.pre_paste(pos, src, dest)
    ...
    local msg = msg ..
                    string.format('%s already exists.  Overwrite?', dest.path)
    local choice = call('confirm',
                        {msg, '&Force\n&No\n&Rename\n&Time\n&Underbar', 0})
    local ret = ''
    if choice == 0 then
        return
...
function M.pre_remove(bufnr, rmfiles)
    ...
    for _, f in ipairs(rmfiles) do msg = msg .. f .. '\n' end
    ...
    for i, action in ipairs(action_list) do
        ...
        if action_set[op] then
            if op == 'call' then
                str = str ..
                          string.format(
                              [[:<C-U>lua tree.call(tree.callback["%s"])<CR>]],
                              vim.fn.escape(lhs, '\\'))
            else
                if expr then
                    str = str .. string.format(
                              [[:<C-u>call v:lua.call_async_action(%s, luaeval('tree.callback["%s"]()'))<CR>]],
                              fn.string(op), vim.fn.escape(lhs, '\\'))
                else
                    str = str ..
                              string.format(
                                  [[:<C-u>call v:lua.call_async_action(%s, %s)<CR>]],
                                  fn.string(op), fn.string(args))
                end
            end
...
function M.print_error(s)
    a.nvim_command(string.format(
                       "echohl Error | echomsg '[tree] %s' | echohl None",
                       M.string(s)))
end
...
function __parse_options(cmdline)
    ...
    for _, s in ipairs(vim.fn.split(cmdline,
                                    __re_unquoted_match([[\%(\\\@<!\s\)\+]]))) do
        local arg = vim.fn.substitute(s, [[\\\( \)]], [[\1]], 'g')
        local arg_key = vim.fn.substitute(arg, [[=\zs.*$]], '', '')

        local name = vim.fn.substitute(vim.fn.tr(arg_key, '-', '_'), [[=$]], '',
                                       ''):sub(2)
        local value
...
function __expand(path)
    if path:find('^~') then path = vim.fn.fnamemodify(path, ':p') end
...
function map_filter(func, t)
    ...
    local rettab = {}
    for k, v in pairs(t) do if func(k, v) then rettab[k] = v end end
    return rettab
end
function __expand_complete(path)
    ...
    elseif vim.fn.match(path, [[^\$\h\w*]]) ~= -1 then
        path = vim.fn
                   .substitute(path, [[^\$\h\w*]], [[\=eval(submatch(0))]], '')
    end
    return __substitute_path_separator(path)
end
function complete(arglead, cmdline, cursorpos)
    ...
    if arglead:find('^-') then
        ...
        local bool_options = vim.tbl_keys(
                                 map_filter(
                                     function(k, v)
                    return type(v) == 'boolean'
                end, copy(user_options())))
        local bt = vim.tbl_map(function(v)
            return '-' .. vim.fn.tr(v, '_', '-')
        end, copy(bool_options))
        vim.list_extend(_, bt)
        local string_options = vim.tbl_keys(
                                   map_filter(
                                       function(k, v)
                    return type(v) ~= 'boolean'
                end, copy(user_options())))
        local st = vim.tbl_map(function(v)
            return '-' .. vim.fn.tr(v, '_', '-') .. '='
        end, copy(string_options))
        vim.list_extend(_, st)

        -- Add "-no-" option names completion.
        local nt = vim.tbl_map(function(v)
            return '-no-' .. vim.fn.tr(v, '_', '-')
        end, copy(bool_options))
        ...
        local files = vim.tbl_filter(function(v)
            return vim.fn.stridx(v:lower(), al:lower()) == 0
        end, vim.tbl_map(__substitute_path_separator,
                         vim.fn.glob(arglead .. '*', true, true)))
        files = vim.tbl_map(__expand_complete, vim.tbl_filter(
                                function(v)
                return vim.fn.isdirectory(v) == 1
            end, files))
        if arglead:find('^~') then
            local home_pattern = '^' .. __expand_complete('~')
            files = vim.tbl_map(function(v)
                return vim.fn.substitute(v, home_pattern, '~/', '')
            end, files)
        end
        files = vim.tbl_map(function(v)
    ...
    return vim.fn.uniq(vim.fn.sort(vim.tbl_filter(
                                       function(v)
            return vim.fn.stridx(v, arglead) == 0
        end, _)))
end
-- Test case
...
function __eval_cmdline(cmdline)
    ...
    while eval_pos >= 0 do
        ...
        cl = cl ..
                 vim.fn.escape(vim.fn.eval(
                                   cmdline:sub(eval_pos + 2, prev_match - 1)),
                               [[\ ]])

        eval_pos = vim.fn.match(cmdline, [[\\\@<!`.\{-}\\\@<!`]], prev_match)
    end
    if prev_match >= 0 then cl = cl .. cmdline:sub(prev_match + 1) end
...
function M.linux() return is_linux end
function M.windows() return is_windows end
function M.macos() return is_macos end
-- Open a file.
function M.open(filename)
    ...
    if tree.windows() then
        ...
        a.nvim_command(printf(
                           "silent execute '!start rundll32 url.dll,FileProtocolHandler %s'",
    ...
    elseif exists('$KDE_FULL_SESSION') == 1 and vim.env['KDE_FULL_SESSION'] ==
        'true' then
        -- KDE.
...
local function init_context(user_context)
    ...
    if vim.tbl_isempty(local_custom.column) then local_custom.column = nil end
...
function M.start(_paths, user_ctx)
    ...
    if #paths == 0 then paths = {fn.getcwd()} end
    if M.alive_buf_cnt < 1 or user_ctx.new then
        ...
        for k, _ in pairs(etc) do if ctx[k] then etc[k] = ctx[k] end end
...
function M.is_directory() return
    fn.get(M.get_candidate(), 'is_directory', false) end
